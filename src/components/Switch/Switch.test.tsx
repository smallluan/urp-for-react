...
describe('Switch 组件', () => {
  ...
  it('渲染默认 Switch 组件', async () => {
    ...
    expect(urpSwitch).toHaveClass('u-switch')
    expect(urpSwitch).toHaveClass('u-switch-state-close')
    expect(urpSwitch).toHaveClass('u-switch-shape-round')
    expect(urpSwitch).toHaveClass('u-switch-size-normal')
    expect(urpSwitch).not.toHaveClass('u-switch-state-open')
    expect(urpSwitch).not.toHaveClass('u-switch-disabled')
    // 模拟点击开关
    await user.click(urpSwitch)
    expect(urpSwitch).toHaveClass('u-switch-state-open')
    expect(urpSwitch).not.toHaveClass('u-switch-state-close')
  })
  // 测试方形 Switch
  it('方形 Switch', () => {
  ...
  it('内置描述生效', async () => {
    ...
    const closeText = urpSwitch.querySelector('.u-desc-inner')
    expect(closeText).toHaveTextContent('关')
    // 打开开关
    await user.click(urpSwitch)
    // 查看打开状态下滑块内部是否显示正确的描述文字
    const openText = urpSwitch.querySelector('.u-desc-inner')
  ...
  it('外置描述生效', async () => {
    ...
    const closeText = urpSwitch.querySelector('.u-desc-outer')
    expect(closeText).toHaveTextContent('关')
    // 打开开关
    await user.click(urpSwitch)
    // 查看打开状态下滑块外部是否显示正确的描述文字
    const openText = urpSwitch.querySelector('.u-desc-outer')
    expect(openText).toHaveTextContent('开')
  })
})
